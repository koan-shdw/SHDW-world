...
**F2 SKIN + ACTION RING BUILT 2026-09-06**, live on main. Owner's correction that day: "when you click on an art an action ring comes up, important for controller use; the green cyberpunk shit, why is settings in a green box": the skin had been queued behind F1, wrong order, fixed now. Built: theme `PLAY` default (Messenger's palette), Nunito self-hosted, rounded chunky shapes, 2 px ink outlines, hard shadows that collapse on press, drawn icons (`Icon.svelte`), title card (`press anywhere to enter`), crosshair that grows to a ring over a touchable, prompt pills, the bar, the hands card, options, toasts, maps, all restyled; `Ring.svelte`: click a work (or E) → the ring of actions on it (move · take down · swap · align wall / turn · look · done), aimed by mouse deltas while locked, the cursor when free, the right stick on a pad; keys 1-5; click / A confirms; the sculpture look rows moved to a `look` panel off the ring. Gamepad in `input.ts`: left stick walks, right stick looks or aims, A do, B put back, X touch, Y turn, bumpers cycle, start menu, back map. NOT built: the ring on bar slots (click = hold, right click = put back / remove there). PROVEN by screenshot on localhost: title card, pill, ring open, hot slice, confirm → in hand.

**09-06 late, his notes, built and live:** (1) the wall widget was a card the locked mouse could not click → it is a readout of keys and wheel: wheel / up down = height (shift 10), left right = snap line, g = gap 0·5·10·20; the changed value lights; [ ] and 1-0 swap the held work, the wheel no longer does. (2) Sounds removed entirely (his word), the sound tab too. (3) The word `play` is gone from everything he sees: theme `WORLD`, tab `controls`, `click to enter`. (4) The ring is Zelda's, his pick from five mockups: round tiles on a dashed orbit, the aimed tile grows yellow, its name on a pill above, the room dims 28 % and the feet freeze while it is open.
